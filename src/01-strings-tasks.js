...
function extractNameFromTemplate(string) {
  return string.substr(7).slice(0, -1);
}
...
function getRectangleString(width, height) {
  const template = `│${' '.repeat(width - 2)}│\n`;
  return `┌${'─'.repeat(width - 2)}┐\n${template.repeat(height - 2)}└${'─'.repeat(width - 2)}┘\n`;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (msg) => cipher[alpha.indexOf(msg)]);
}
